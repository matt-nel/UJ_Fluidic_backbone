...
class SetupGUI:
    def __init__(self, primary):
        ...
        self.syringe_volumes = [0, 1, 2, 4, 5, 10, 20]
    ...
    def init_utilities_panel(self):
        run_fb_button = tkinter.Button(self.utilities_frame, text='Run Fluidic Backbone', font=self.fonts['buttons'], bg='DeepSkyBlue', fg='black', command=self.run_fb)
    ...
    def setup_valve(self):
        def accept():
            motor = motor_connector.get()
            try:
                hall_sensor = self.pins[hall_connector.get()]
            except KeyError:
        ...
        hall_sensor_options = tkinter.OptionMenu(valve_setup, hall_connector, 'Analog Read 1 (A3)', 'Analog Read 2 (A4)')
        ...
        hall_sensor_options.grid(row=8, column=0)
    ...
    def setup_motor(self, name, stepper_name, motor, mod_config):
        ...
        self.modules[name] = {'mod_config': mod_config, 'devices': motor_config}
    ...
    def run_fb(self):
        def start_fb():
            ...
            fb_primary = tkinter.Tk()
            fb_gui = FLuidicBackboneUI(self.primary, self.simulation)
            fb_primary.mainloop()
